...
async function main() {
  ...
  const cinderToken = await (await FT.deploy(
  ...
  d.flow = await cinderToken.getAddress(); console.log(`CinderToken: ${d.flow}`);
  ...
  await (await cinderToken.transfer(d.timelock, 250_000_000n * 10n**18n)).wait();
  ...
  await (await cinderToken.transfer(d.mining, 450_000_000n * 10n**18n)).wait();
